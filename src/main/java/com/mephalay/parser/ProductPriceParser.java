...
/**
 * Let code duplication flow, don't refactor code duplication, parse methods must be independent.
...
public class ProductPriceParser {
    ...
    public static List<ParserVatanProduct> parseForVatanMonitorPrices() {
        List<ParserVatanProduct> vatanProductList = new ArrayList<>();
        try {
            CloseableHttpClient httpClient = HttpClients.createDefault();
            HttpGet getMethod = new HttpGet("http://www.vatanbilgisayar.com/bilgisayar-monitorleri/?ps=14560");
            HttpResponse response = httpClient.execute(getMethod);
            BufferedReader rd = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
            StringBuilder result = new StringBuilder();
            String line = "";
    ...
    public static List<ParserVatanProduct> parseForVatanLaptopPrices() {
        ...
        try {
            CloseableHttpClient httpClient = HttpClients.createDefault();
            HttpGet getMethod = new HttpGet("http://www.vatanbilgisayar.com/notebook/?ps=14560");
            HttpResponse response = httpClient.execute(getMethod);
            BufferedReader rd = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
            StringBuilder result = new StringBuilder();
            String line = "";
    ...
    public static List<ParserVatanProduct> parseVatanPhoneList() {
        List<ParserVatanProduct> vatanProductList = new ArrayList<>();
        try {
            CloseableHttpClient httpClient = HttpClients.createDefault();
            HttpGet getMethod = new HttpGet("http://www.vatanbilgisayar.com/cep-telefonu-modelleri/?ps=14560");
            HttpResponse response = httpClient.execute(getMethod);
            BufferedReader rd = new BufferedReader(new InputStreamReader(response.getEntity().getContent()));
            StringBuilder result = new StringBuilder();
            String line = "";
